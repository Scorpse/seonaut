...
const reserveAPIArchiveJobSQL = `INSERT INTO api_export_jobs (id, tenant_id, project_id, crawl_id, kind, state, created_at, updated_at) VALUES (?, ?, ?, ?, 'archive.wacz', 'pending', ?, ?) ON DUPLICATE KEY UPDATE updated_at = updated_at`

const getAPIArchiveJobSQL = `SELECT id, tenant_id, project_id, crawl_id, kind, state, COALESCE(artifact_path, ''), artifact_size, COALESCE(failure_code, ''), created_at, updated_at, finished_at FROM api_export_jobs WHERE tenant_id = ? AND project_id = ? AND crawl_id = ? AND kind = 'archive.wacz' LIMIT 1`
...
func (r APIExportRepository) ReserveArchive(ctx context.Context, principal api.Principal, projectID, crawlID string) (APIExportJob, error) {
	...
	now := r.now()
	if _, err := r.DB.ExecContext(ctx, reserveAPIArchiveJobSQL, id, principal.TenantID, projectID, crawlID, now, now); err != nil {
		return APIExportJob{}, err
	}
...
func (r APIExportRepository) CompleteArchive(ctx context.Context, principal api.Principal, projectID, crawlID, path string, size int64) error {
	now := r.now()
	_, err := r.DB.ExecContext(ctx, completeAPIArchiveJobSQL, path, size, now, now, principal.TenantID, projectID, crawlID)
	return err
}

func (r APIExportRepository) FailArchive(ctx context.Context, principal api.Principal, projectID, crawlID, code string) error {
	now := r.now()
	_, err := r.DB.ExecContext(ctx, failAPIArchiveJobSQL, code, now, now, principal.TenantID, projectID, crawlID)
	return err
}
...
func scanAPIExportJob(row apiExportJobScanner) (APIExportJob, error) {
	...
	err := row.Scan(&job.ID, &job.TenantID, &job.ProjectID, &job.CrawlID, &job.Kind, &job.State, &job.ArtifactPath, &job.ArtifactSize, &job.FailureCode, &job.CreatedAt, &job.UpdatedAt, &finished)
	if errors.Is(err, sql.ErrNoRows) {
		return APIExportJob{}, api.ErrCrawlNotFound
	}
	if err != nil {
